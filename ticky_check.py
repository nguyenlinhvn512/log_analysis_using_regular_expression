...
error_messages = {}
per_user = {}
logfile = r"/mnt/c/Users/Utilisateur/Documents/GitHub/log_analysis_using_regular_expression/syslog.log"
...
with open(logfile, "r") as f:
        for line in f:
                ...
                if result.groups()[0] == "ERROR":
                        ...
                        message = result.groups()[1]
                        name = str(result.groups()[2])[1:-1]
                        error_messages[message] = error_messages.get(
                            message, 0) + 1
                        if name in per_user:
                                user = per_user[name]
                                user[category] += 1
...
sorted_messages = [("Error", "Count")] + sorted(error_messages.items(),
...
with open("user_statistics.csv", "w") as user_file:
        for line in sorted_users:
                if isinstance(line[1], dict):
                        user_file.write("{}, {}, {}\n".format(
                            line[0], line[1].get("INFO"), line[1].get("ERROR")))
                else:
                        user_file.write("{}, {}, {}\n".format(
